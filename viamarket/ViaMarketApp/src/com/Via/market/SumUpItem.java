...
public class SumUpItem extends Activity {
	...
	// String urlRequestImages =
	// "http://viamarket-001-site1.myasp.net/api/item/{item:id}/image/upload/";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			listUri.add(this.getRealPathFromURI(this,
					Uri.parse(this.getIntent().getStringExtra("pict0"))));

			String url1 = this.getIntent().getStringExtra("pict1").toString();
			this.getBitmap(url1, im1);
			listUri.add(this.getRealPathFromURI(this,
					Uri.parse(this.getIntent().getStringExtra("pict1"))));

			String url2 = this.getIntent().getStringExtra("pict2").toString();
			this.getBitmap(url2, im2);
			listUri.add(this.getRealPathFromURI(this,
					Uri.parse(this.getIntent().getStringExtra("pict2"))));
	...
	public void getBitmap(String str, ImageView iv) {
		...
		Bitmap bmp = ImageLoader.getInstance().loadImageSync(str, targetSize,
	...
	public void addListener(ImageView iv) {
		iv.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				try {
					Intent intent = new Intent(SumUpItem.this,
							ImagePagerActivity.class);

					String[] urls = SumUpItem.this.listUri
							.toArray(new String[SumUpItem.this.listUri.size()]);
					...
					System.out.println("No pictures to display");
				}
			}
		});
	}
	...
	// find if the picture has been taken in landscape or portrait and return
	// the rigth rotation angle
